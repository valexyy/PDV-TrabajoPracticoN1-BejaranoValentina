class PinkBall {
  ...
  void display() {
    // CORRUPCIÓN: Efectos visuales extraños
    if (invulnerable && (invulnerabilityTimer / 10) % 2 == 0) {
      fill(255, 182, 193, 100);
    } else {
      // Efecto de corrupción ocasional
      if (frameCount % 300 < 10) {
        fill(255, 0, 0, 200); // Rojo parpadeante
        stroke(128, 0, 0);
      } else {
        fill(255, 182, 193);
        stroke(255, 105, 180);
    ...
    // CORRUPCIÓN: Forma distorsionada ocasionalmente
    if (frameCount % 500 < 15) {
      // Dibujar forma extraña
      beginShape();
      vertex(x-30, y-15);
      vertex(x+30, y-15);
      vertex(x+20, y+15);
